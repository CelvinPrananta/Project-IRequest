...
const AnnouncementFlyer = () => {
  ...
  useEffect(() => {
    ...
    const flyersCollectionRef = collection(db, `flyers/${churchId}/flyer_list`);
    const q = query(flyersCollectionRef);
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        flyerData.push({ id: doc.id, ...doc.data() });
      });
  
      // Perbarui state dengan daftar flyer
      setFlyers(flyerData);
      setLoading(false);
    }, (error) => {
  ...
  const validateFields = () => {
    let fieldErrors = {};
    if (!newFlyer.filename) fieldErrors.image = translations[language].image_required;
  ...
  const handleAddFlyer = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // Iterasi setiap flyer dalam newFlyer
        const uploadPromises = newFlyer.map(async (flyer) => {
          // Generate unique ID untuk flyer
          const flyerId = uuidv4();

          // Ambil nama file asli tanpa timestamp atau prefix tambahan
          const fileNameOnly = flyer.filename.replace(/^\d+_/, "");

          // Referensi koleksi flyer_list dalam Firestore
          const flyerRef = doc(db, `flyers/${churchId}/flyer_list`, flyerId);

          // Tambahkan flyer baru ke state
          const flyerData = { id: flyerId, filename: fileNameOnly, image: flyer.image, createdAt };

          // Simpan flyer sebagai dokumen terpisah dalam subkoleksi
          await setDoc(flyerRef, flyerData);
          return flyerData;
        });

        // Tunggu semua proses unggah selesai
        const uploadedFlyers = await Promise.all(uploadPromises);

        setFlyers([...flyers, ...uploadedFlyers]); // Tambahkan semua flyer ke state
        setNewFlyer([]); // Setel ulang input setelah sukses
  ...
  const handleImageUpload = async (acceptedFiles) => {
    ...
    for (const file of acceptedFiles) {
      ...
      try {
        ...
        reader.onload = () => {
          setNewFlyer((prevFlyers) => [...prevFlyers, { image: reader.result, filename }]); // Tampilkan gambar lokal sementara
        };
  ...
  const filteredFlyers = useMemo(() => {
    ...
    return flyers.filter(flyer =>
      Object.values(flyer).some(value =>
  ...
  const sortedFlyers = useMemo(() => {
    if (!Array.isArray(filteredFlyers) || filteredFlyers.length === 0) return [];
    return [...filteredFlyers].sort((a, b) => {
  ...
  }, [filteredFlyers, sortConfig]);

  // Untuk membuat paginasi
  const pageCount = Math.ceil(sortedFlyers.length / itemsPerPage);
  ...
  return (
    <CCol xs={12}>
      ...
      <CCard className="mb-4">
        <CCardHeader>
          <strong>{translations[language].flyer} - {formattedChurchSlug}</strong>
        ...
        <CCardBody>
          ...
          <div className="table-responsive mt-3">
            <CTable hover>
              ...
              <CTableBody>
                {Array.isArray(sortedFlyers) && sortedFlyers.length > 0 ? (
                  sortedFlyers.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((flyer, index) => (
                    <CTableRow key={flyer.id}>
                      <CTableDataCell>{index + 1}</CTableDataCell>
                      <CTableDataCell>
                        {flyer.image ? (
                          <Link to={flyer.image} data-fancybox="gallery" data-caption={flyer.filename || 'No Filename'}>
                            <img className='preview-flyer' src={flyer.image} alt={flyer.filename || 'No Filename'} />
                          </Link>
                        ) : (
                          <span>{translations[language].no_image}</span>
                        )}
                      </CTableDataCell>
                      <CTableDataCell>{flyer.filename || 'No Filename'}</CTableDataCell>
          ...
          <div className="d-flex justify-content-between align-items-center mt-3">
            ...
            <span className="text-muted">
              {flyers.length > 0 ? `${translations?.[language]?.showing} ${currentPage * itemsPerPage + 1} ${translations?.[language]?.to} ${Math.min((currentPage + 1) * itemsPerPage, flyers.length)} ${translations?.[language]?.of} ${flyers.length} ${translations?.[language]?.entries}`: `${translations?.[language]?.showing_not_data}`}
            </span>
      ...
      <CModal visible={showModal} onClose={() => setShowModal(false)}>
        ...
        <CModalBody>
          <CForm>
              ...
              {(newFlyer.length > 0 || uploadProgress > 0) && (
                <div className='preview-container'>
                  {newFlyer.map((flyer, index) => (
                    <Link key={index} className='link' to={flyer.image} data-fancybox="gallery" data-caption={fileDetails[index]?.name || "Preview Image"}>
                      <img className='img' src={flyer.image} alt="Preview" />
          ...
          <span className='size-format'>{translations[language].nb}<br />{translations[language].size_format_768}</span>
        </CModalBody>
        <CModalFooter>
          {/* Tombol Reset */}
          {newFlyer.length > 0 && (
            <CButton color="warning button-reset" onClick={() => { setNewFlyer([]); setFileDetails([]); setUploadProgress(0) }}>
          ...
          <CButton color="primary" onClick={() => { handleAddFlyer(); setFileDetails({ name: '', size: '' }); setNewFlyer({ image: '', filename: '', createdAt: '' }); }} disabled={ loading || (uploadProgress > 0 && uploadProgress < 100) } >
      ...
      <CModal visible={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
        ...
        <CModalFooter>
          ...
          <CButton color="danger button-confirmation" onClick={async () => {
              if (!flyerToDelete || !churchId) return;

              setLoading(true); // Aktifkan loading

              setTimeout(async () => {
                try {
                  // Referensi ke flyer di subkoleksi Firestore
                  const flyerRef = doc(db, `flyers/${churchId}/flyer_list/${flyerToDelete}`);

                  // Hapus flyer dari Firestore
                  await deleteDoc(flyerRef);

                  // Hapus flyer dari state lokal
                  setFlyers((prevFlyers) => prevFlyers.filter((flyer) => flyer.id !== flyerToDelete));
